...
import { bookToUpdate } from '../REDUX/filter/actions';

const Card = ({ book, setUpdateForm }) => {
    const dispatch = useDispatch();
    const { name, author, thumbnail, price, rating, featured, id } = book;
    const ratings = [...Array(rating).keys()];

    const handleDataToEdit = () => {
        setUpdateForm(true);
        dispatch((bookToUpdate({ name, author, thumbnail, price, rating, featured, id })))
    }
            ...
            <div className="flex-1 h-full pr-2 pt-2 flex flex-col">
                <div className="flex items-center justify-between">
                    ...
                    <div className="text-gray-500 space-x-2">
                        <button className="lws-edit" onClick={handleDataToEdit}>
                ...
                <div className="space-y-2 mt-4 h-full">
                    ...
                    <div className="lws-stars">
                        {
                            ratings.length > 0 && ratings.map((el) =>
                                <svg key={el} viewBox="0 0 20 20" fill="currentColor" className="lws-star">
